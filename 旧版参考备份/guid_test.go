...
func TestGUID(t *testing.T) {
	for i := 0; i < len(guidFixtures); i++ {
		...
		if f.ShouldMatch && guid.String() != strings.ToUpper(f.S) {
			t.Errorf("GUID test \"%v\" (%v of %v) failed. Expected \"%v\" from GUID.I取文本. Received \"%v\" instead.", f.Name, i, len(guidFixtures), strings.ToUpper(f.S), guid)
		}
	}
}
