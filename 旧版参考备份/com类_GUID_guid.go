...
// GUID 是Windows API特定的GUID类型。
//
// 这与COM的直接传递的Windows GUID类型匹配。
...
// NewGUID 将给定字符串转换为符合Windows API的全局唯一标识符。
//
// 提供的字符串可以是以下任何格式：
//
...
// String 将GUID转换为字符串形式。它将遵循以下模式：
//
//	{XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX}
//
// 如果GUID为空，则返回空GUID的字符串表示：
//
...
// I是否相等_GUID  IsEqualGUID 比较两个GUID。
//
// 非恒定时间比较。
func I是否相等_GUID(guid1 *GUID, guid2 *GUID) bool {
